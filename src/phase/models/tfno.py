...
from .layers import FactorizedSpectralConv3d
...
class FullyConnected(nn.Module):
    def __init__(
        ...
        if num_layers <= 1:
            ...
            layers = [nn.Linear(in_features, layer_size), activation]
            for _ in range(num_layers - 2):
                layers.extend([nn.Linear(layer_size, layer_size), _get_activation(activation_fn)])
...
class TFNO(nn.Module):
    ...
    def getTFNOEncoder(self):
        ...
        if self.dimension == 3:
            ...
            raise NotImplementedError("Invalid dimensionality.")
